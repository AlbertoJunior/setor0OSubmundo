...
5. **PROIBIÇÃO DE COMMIT NO PROJETO PRINCIPAL:** Você está expressamente **PROIBIDO** de propor ou executar comandos de modificação Git (como `git commit`, `git push`, `git rebase`, etc.) na raiz do repositório principal do projeto. Sua atuação e automação com o Git é estritamente e exclusivamente restrita ao repositório interno da pasta `.agent` (conforme descrito na regra 2).
